...
#    Then delete the same project
  ...
  Scenario Outline: Company domain change (Already taken by another verified company)
    ...
    And Add a '<domain>' already taken by another company
    Then Verify updated '<domain>'
    Then Unverify the company
    Examples:
      | domain    |
      | gmail.com |

  Scenario Outline: Company domain change (Already taken by another unverified company)
    Given login as super-admin
    Then register a company
    When Edit company details
    And Add a '<domain>' already taken by another company
    Then Verify '<domain>' update
    Then Unverify the company
    Examples:
      | domain   |
      | temp.com |

  Scenario: Company domain change (Domain not used by any other company)
    Given login as super-admin
    Then register a company
    When Edit company details
  ...
  Scenario: Clear all domain validation
    Given login as super-admin
    Then register a company
    When Edit company details
